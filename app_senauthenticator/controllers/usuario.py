...
#         except Usuario.DoesNotExist:
#             return JsonResponse({'error': f"Ningun usuario con este correo '{email}' encontrado"}, status=404)
...
#         user = Usuario.objects.get(numero_documento_usuario=request.data['numero_documento_usuario'])
...
#         # Crear respuesta con el usuario serializado
#         response = Response({'user': serializer.data}, status=status.HTTP_200_OK)
...
#             secure=True if not settings.DEBUG else False,  # Solo HTTPS en producción
#             samesite='None',  # Cambiar según la configuración del dominio
#             max_age=3600,  # Expiración en 1 hora
#         )
...
# #         response = Response({'message': 'Sesión cerrada y token eliminado.'}, status=status.HTTP_200_OK)
